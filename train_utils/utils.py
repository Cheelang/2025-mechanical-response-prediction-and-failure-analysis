...
class ConfusionMatrix(object):
    ...
    def compute(self):
        ...
        # 计算每个类别预测与真实目标的iou
        iou = (torch.diag(h) / (h.sum(1) + h.sum(0) - torch.diag(h))).cpu().numpy()

        miou = iou.mean().item()

        F1 = 2 * (Precision * recall) / (Precision + recall)

        return acc_global,iou,miou,recall,Precision,F1
...
class DiceCoefficient(nn.Module):
    def __init__(self, num_classes, ignore_index: int = -100):
